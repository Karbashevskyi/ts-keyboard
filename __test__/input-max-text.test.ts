...
import {AlphabetEnum} from 'ts-keycode/lib/enums/alphabet.enum';
...
describe('input:text', () => {
    ...
    it.each(Object.keys(keycodeSegments.alphabet).map((a: any) => [a, keycode[a]]))('Should true for %s / %s', (key: any, keyCode: any) => {
    ...
    it.each(Object.keys(keycodeSegments.numbers).map((a: any) => [a, keycode[a]]))('Should false for %s / %s', (key: any, keyCode: any) => {
        input.dispatchEvent(new KeyboardEvent("keydown", {
            keyCode: keyCode,        // example values.
            which: keyCode,
            shiftKey: false,    // you don't need to include values
